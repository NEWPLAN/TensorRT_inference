...
{
    CVReader reader;
    pixelMx *da = reader.load_one();
    for (int index = 10; index > 0; index--)
    {
        reader.load_one();
    }
    std::vector<char> data((char *)(da->dataptr), (char *)(da->dataptr) + da->size_in_byte);
    cv::Mat src = cv::imdecode(cv::Mat(data), CV_LOAD_IMAGE_COLOR);
    cv::Mat dst;
    cv::resize(src, dst, cv::Size(256, 256), (0, 0), (0, 0), cv::INTER_LINEAR);
    LOG(INFO) << "Image info: " << std::endl
    ...
    case CV_8U:
    {
        ...
        //}
        LOG(INFO) << "Image depth is CV_8U";
        break;
    }
    case CV_32F:
    {
        LOG(INFO) << "Image depth is CV_32F";
        break;
    }
    case CV_64F:
    {
        LOG(INFO) << "Image depth is CV_64F";
        break;
    }
    default:
    {
...
int main(int argc, char **argv)
{
    ...
    // create a TensorRT model from the caffe model and serialize it to a stream
    {
        ...
        LOG(INFO) << "deviceCount := " << num << std::endl;
        for (int i = 0; i < num; i++)
        {
    ...
    {
        std::thread t1(infer_engine, index);
        tp.push_back(std::move(t1));
    }
    for (auto &i : tp)
    {
        i.join();
